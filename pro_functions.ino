...
while(digitalRead(select_btn) == HIGH)      //Till Ok button(butt_2) is pressed!
  {
    if(digitalRead(change_btn) == LOW)    //Mode select krne wala button i.e butt_1
      {
        delay(300);
    ...
    lcd.clear();
    lcd.print("YOUR CHOICE: ");                      //Submit krne tak continuously print krta rhega
    lcd.print(count);
    delay(50);
  }
    switch(count)
...
void countdown()
{
        ...
        while(digitalRead(select_btn) == HIGH)      //jab tak Ok button press nahi hoga
          {
            if(digitalRead(change_btn) == LOW)    //jab tak user button1 press krega hour ki value increment hogi
              {
                delay(300);
                hour++;
              }
            ...
            lcd.clear();
            lcd.print(hour);                            //continuosly print hogi hour ki value
            delay(50);
          }
        ...
        int e2 = minute;
        int e3 = 0;
         for(int e1 = hour; e1 >= 0; e1--)            //yaha se shuru timer print krna
           {
              for(e2 ; e2 >=0; e2--)
              {
                for(e3 ; e3 >= 0; e3--)        //mast logic h ,try kr samjhne ka
                  {
                    digitalWrite(8,HIGH);
                    lcd.clear();
                    lcd.print("Time Left:");
